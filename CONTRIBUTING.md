...
The default build includes the `otel` feature. Add `--no-default-features` to exclude the OpenTelemetry SDK and exporter. The `[otel].enabled` configuration value controls telemetry at runtime.
...
- `make test_e2e` - the spawn-the-binary end-to-end suite (`crates/tests`, `--features e2e,otel`): forks workers, binds ports, drives real HTTP, and checks lifecycle and telemetry behavior. Run it with one test thread after `test_nts`.
...
cargo clippy -p tests --features e2e,otel --tests -- -D warnings
```
...
| `crates/plugins/otel`  | native telemetry, context propagation, and the isolated OTLP exporter               |
