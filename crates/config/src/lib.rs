...
mod tests {
    ...
    fn http_pool_resolves_from_file() {
        ...
        .unwrap();
        let http = merge(file, Some(Path::new("/etc/rapira")))
            .unwrap()
            .http
            .unwrap();
    ...
    fn server_port_derives_from_listen_and_mb_converts() {
        ...
        .unwrap();
        let http = merge(file, Some(Path::new("/w"))).unwrap().http.unwrap();
    ...
    fn file_entrypoint_is_config_dir_relative() {
        ...
        let http = merge(file, Some(Path::new("/srv/app")))
            .unwrap()
            .http
            .unwrap();
        assert_eq!(
    ...
    fn entrypoint_is_required() {
        ...
        assert!(
            err.contains("no plugin configured: add an [http] or a [grpc] table"),
            "{err}"
        );
